    ...
    <div className="grid gap-6 sm:grid-cols-3">
      {statCards.map((stat, index) => (
        <motion.div
          ...
          className="group relative overflow-hidden rounded-2xl border border-border bg-card/80 p-8 backdrop-blur-sm transition-all hover:border-border hover:bg-card"
        >
          <div className="flex flex-col items-center text-center">
            <div className="mb-4 flex items-baseline gap-2">
              <p className="text-6xl font-black text-foreground">{stat.value}</p>
              {stat.unit && <span className="text-lg text-muted-foreground">{stat.unit}</span>}
            </div>
            <p className="text-sm font-medium text-muted-foreground">{stat.label}</p>
            {stat.subValue && <p className="mt-1 text-xs text-muted-foreground">{stat.subValue}</p>}
          </div>
